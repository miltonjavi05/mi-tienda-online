...
interface Sale { id: string; name: string; phone: string; product: string; amount: number; date: string; createdAt: number; }
interface Investment { id: string; desc: string; amount: number; date: string; createdAt: number; }
interface AgendaClient { id: string; name: string; phone: string; product: string; amount: number; paidAmount: number; paid: boolean; notes: string; createdAt: number; }
...
function MamiPanel({ onLogout }: { onLogout: () => void }) {
 ...
 // Carga inicial desde Firestore — así se ve lo mismo sin importar desde qué dispositivo inicie sesión
    ...
    (async () => {
      try {
        ...
        const s: Sale[] = rawSales.map(r => ({
          ...
          product: (r.product as string) || "", amount: Number(r.amount) || 0,
        ...
        const a: AgendaClient[] = rawAgenda.map(r => ({
          id: r.id, name: (r.name as string) || "", phone: (r.phone as string) || "",
          product: (r.product as string) || "", amount: Number(r.amount) || 0,
          paidAmount: Number(r.paidAmount) || 0, paid: !!r.paid, notes: (r.notes as string) || "",
  ...
  const addSale = useCallback(async (data: Omit<Sale, "id" | "createdAt">) => {
    ...
    try {
      const realId = await fsAddToCollection(COL.ventas, { ...data, createdAt });
  ...
  const addAgenda = useCallback(async (data: Omit<AgendaClient, "id" | "createdAt" | "paid" | "paidAmount">) => {
    ...
    try {
      const realId = await fsAddToCollection(COL.agenda, { ...data, createdAt, paid: false, paidAmount: 0 });
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        {tab === "agenda" && <AgendaTab agenda={agenda} addAgenda={addAgenda} removeAgenda={removeAgenda} togglePaid={toggleAgendaPaid} rate={rate} />}
...
function VentasTab({ sales, addSale, removeSale, rate }: {
  ...
  const add = () => {
    ...
    if (!name.trim() || !product.trim() || !amt || amt <= 0) { setErr("Completa nombre, producto y un monto válido."); return; }
    addSale({ name: name.trim(), phone: phone.trim(), product: product.trim(), amount: amt, date });
    setName(""); setPhone(""); setProduct(""); setAmount(""); setErr("");
  };
  ...
  return (
    <>
      <Card>
        ...
        <Field label="Producto"><input style={inputStyle} value={product} onChange={e => setProduct(e.target.value)} placeholder="Ej: Detergente 1L + Desinfectante" /></Field>
        ...
        {!!amount && <p style={{ margin: "0 0 0.6rem", fontSize: 11.5, color: "#c2447a" }}>≈ {fmtBs(parseFloat(amount) * rate)}</p>}
      ...
      {grouped.length === 0 ? (
        ...
        <div key={d} style={{ marginBottom: "0.9rem" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#c288a0", margin: "0 0 0.4rem 0.2rem" }}>
            {fmtDate(d)} · {fmtUSD(items.reduce((a, s) => a + s.amount, 0))}
          </p>
          {items.map(s => (
...
function InversionTab({ invs, addInv, removeInv }: {
  ...
  const add = () => {
    ...
    addInv({ desc: desc.trim(), amount: amt, date });
      ...
      {sorted.length === 0 ? (
        ...
        <div key={i.id} style={{ display: "flex", alignItems: "center", gap: 10, background: "#fffdfe", border: "1px solid #ffe0ec", borderRadius: 12, padding: "0.6rem 0.8rem", marginBottom: 6 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ margin: 0, fontSize: 10.5, color: "#c9a3b6" }}>{fmtDate(i.date)}</p>
          </div>
...
function AgendaTab({ agenda, addAgenda, removeAgenda, togglePaid, rate }: {
  agenda: AgendaClient[];
  ...
  const add = () => {
    const amt = parseFloat(amount);
    if (!name.trim() || !amt || amt <= 0) { setErr("Ingresa el nombre y el monto que debe."); return; }
    addAgenda({ name: name.trim(), phone: phone.trim(), product: product.trim(), amount: amt, notes: notes.trim() });
    setName(""); setPhone(""); setProduct(""); setAmount(""); setNotes(""); setErr("");
  };
  ...
  return (
    <>
      <Card>
        ...
        <Field label="Cliente"><input style={inputStyle} value={name} onChange={e => setName(e.target.value)} placeholder="Nombre del cliente" /></Field>
        <Field label="Teléfono"><input style={inputStyle} value={phone} onChange={e => setPhone(e.target.value)} placeholder="Teléfono (opcional)" /></Field>
        <Field label="Producto que se llevó"><input style={inputStyle} value={product} onChange={e => setProduct(e.target.value)} placeholder="Ej: Kit de limpieza completo" /></Field>
        <Field label="Monto que debe (USD)"><input style={inputStyle} type="number" min={0} step={0.01} value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" /></Field>
      ...
      {sorted.length === 0 ? (
        ...
        <div key={c.id} style={{
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            ...
            }}>{c.paid ? "✓ Pagado" : "Marcar como pagado"}</button>
