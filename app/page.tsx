...
async function trackContactWhatsApp(product?: Product, userEmail?: string): Promise<void> {
  const eventId = genEventId();
  const baseParams: Record<string, unknown> = { content_type: "product", content_name: product?.name || "WhatsApp Contact" };
  if (product) { baseParams.content_ids = [product.id]; baseParams.value = product.price; baseParams.currency = "USD"; }
  fbqTrack("Contact", baseParams, { eventID: eventId });
  await sendCAPI("Contact", eventId, { content_ids: product ? [product.id] : undefined, content_name: product?.name, content_type: "product", value: product?.price, currency: "USD" }, { email: userEmail });
}
...
function DraggableWA(){
  ...
  const onClick=useCallback((e:React.MouseEvent)=>{if(moved.current){e.preventDefault();return;}trackContactWhatsApp();window.open(SOCIAL.whatsapp,"_blank","noreferrer");},[]);
    ...
    <div className="pc" style={{WebkitTapHighlightColor:"transparent",touchAction:"manipulation",position:"relative",display:"flex",flexDirection:"column"}}>
      ...
      <button onClick={e=>{e.stopPropagation();onBuyNow();}} style={{background:"linear-gradient(180deg,#ffffff 0%,#ededed 100%)",color:"#080808",border:"none",padding:"6px 0",fontSize:9,fontWeight:900,letterSpacing:1.5,cursor:"pointer",fontFamily:"inherit",borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",gap:4,WebkitTapHighlightColor:"transparent",width:"100%",boxShadow:"0 3px 12px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.6)",marginBottom:"0.3rem"}}>
        ...
        <div style={{display:"flex",gap:"0.4rem"}}>
          <button onClick={e=>{e.stopPropagation();onClick();}} style={{background:"rgba(255,255,255,0.03)",color:"#999",border:"1px solid rgba(255,255,255,0.14)",padding:"2px 0",fontSize:9,fontWeight:700,letterSpacing:1,cursor:"pointer",fontFamily:"inherit",borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",gap:4,WebkitTapHighlightColor:"transparent",flex:1}}>
          ...
          <button onClick={e=>{e.stopPropagation();trackContactWhatsApp(product);window.open(buildConsultUrl(product),"_blank","noreferrer");}} aria-label="Finalizar pedido por WhatsApp" style={{position:"relative",overflow:"hidden",background:"linear-gradient(150deg, rgba(255,255,255,0.20) 0%, rgba(255,255,255,0.05) 55%, rgba(255,255,255,0.16) 100%)",border:"1px solid rgba(255,255,255,0.3)",padding:"2px 0",cursor:"pointer",fontFamily:"inherit",borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",gap:4,WebkitTapHighlightColor:"transparent",flex:1,backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.35), 0 2px 10px rgba(0,0,0,0.4)"}}>
            ...
            <span style={{fontSize:8,fontWeight:800,letterSpacing:1,color:"#fff"}}>FINALIZAR PEDIDO</span>
          </button>
        </div>
    </div>
  );
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"0.3rem"}}>
        ...
        <div style={{display:"flex",gap:"0.3rem"}}>
          <button
            ...
            style={{background:"rgba(255,255,255,0.03)",color:"#777",border:"1px solid rgba(255,255,255,0.12)",padding:"1px 0",fontSize:8,fontWeight:700,letterSpacing:0.8,cursor:"pointer",fontFamily:"inherit",borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",gap:3,WebkitTapHighlightColor:"transparent",flex:1}}
          >
          ...
          >
            <span style={{position:"absolute",top:0,left:"-40%",width:"35%",height:"100%",background:"linear-gradient(115deg,transparent,rgba(255,255,255,0.45),transparent)",transform:"skewX(-18deg)",pointerEvents:"none"}}/>
            <svg width="9" height="9" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
            <span style={{fontSize:8,fontWeight:800,letterSpacing:0.5,color:"#fff"}}>FINALIZAR PEDIDO</span>
          </button>
        </div>
      </div>
    </div>
      ...
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        <div className="fg" style={{display:"grid",gap:"2rem",marginBottom:"2rem"}}>
          ...
          <div>
            ...
            <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noreferrer" style={{display:"inline-flex",alignItems:"center",gap:"0.45rem",background:"#0d1e0d",color:"#4caf50",padding:"0.55rem 0.9rem",borderRadius:8,fontSize:11,fontWeight:700,textDecoration:"none",marginBottom:"0.75rem",border:"1px solid #162516"}} onClick={()=>trackContactWhatsApp()}><IcWA s={12} c="#4caf50"/> WhatsApp</a>
        ...
        <div style={{position:"fixed",inset:0,zIndex:300,display:"flex"}}>
          ...
          <div style={{position:"relative",background:"#0e0e0e",width:272,height:"100%",padding:"2rem 1.5rem",overflowY:"auto",display:"flex",flexDirection:"column",animation:"slideInLeft 0.22s cubic-bezier(0.25,0.46,0.45,0.94)",borderRight:"1px solid #1a1a1a"}}>
            ...
            <div style={{marginTop:"auto",paddingTop:"2rem"}}>
              ...
              <div style={{display:"flex",gap:"0.6rem"}}><a href={SOCIAL.whatsapp} target="_blank" rel="noreferrer" className="sl" style={S.socialA} onClick={()=>trackContactWhatsApp()}><IcWA s={16}/></a><a href={SOCIAL.instagram} target="_blank" rel="noreferrer" className="sl" style={S.socialA}><IcIG s={16}/></a><a href={SOCIAL.facebook} target="_blank" rel="noreferrer" className="sl" style={S.socialA}><IcFB s={16}/></a><a href={SOCIAL.tiktok} target="_blank" rel="noreferrer" className="sl" style={S.socialA}><IcTT s={16}/></a></div>
            </div>
          </div>
        </div>
      )}
