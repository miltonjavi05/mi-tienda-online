...
interface OrderSnapshot { items:CartItem[]; total:number; payMethod:string; deliveryInfo:DeliveryInfo; comprobanteUrl:string; waUrl:string; }

// ── "thankyou" añadido al union ──
type MainView = "fokus"|"shop"|"comunidad"|"cart"|"admin"|"account"|"thankyou";
type ShopFilter = typeof ALL_SHOP_CATS[number]|"TODO"|typeof LENTES_SUBCATS[number];

// ═══════════════════════════════════════════════════════════════════════════════
//  *** META PIXEL / CAPI — CONECTAR AQUÍ ***
//  Llama a esta función justo cuando se muestra la pantalla de gracias.
// ═══════════════════════════════════════════════════════════════════════════════
function fireMetaPurchase(value:number, orderId:string, email?:string) {
  // 1. Pixel client-side — descomentar cuando el snippet fbq esté en layout.tsx
  // if (typeof window !== "undefined" && (window as any).fbq) {
  //   (window as any).fbq("track", "Purchase", { value, currency: "USD", order_id: orderId });
  // }

  // 2. Conversions API server-side — crear /app/api/meta-capi/route.ts y descomentar
  // fetch("/api/meta-capi", {
  //   method: "POST",
  //   headers: { "Content-Type": "application/json" },
  //   body: JSON.stringify({ event_name:"Purchase", value, currency:"USD", order_id:orderId, user_data:{ em: email } }),
  // }).catch(() => {});

  console.log("[FOKUS] Purchase event →", { value, orderId, email });
}
// ═══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  THANK YOU VIEW — Ultra rápida, sin dependencias externas
// ══════════════════════════════════════════════════════════════════════════════
const ThankYouView = memo(function ThankYouView({
  ...
  // Meta pixel — se dispara una sola vez
  useEffect(() => {
    if (fired.current) return;
    fired.current = true;
    const orderId = "FKS-" + Date.now().toString(36).toUpperCase();
    fireMetaPurchase(order.total, orderId);
  }, [order.total]);

  // Confetti canvas — canvas en null-check para SSR
  useEffect(() => {
  ...
  const firstName = order.items[0]?.product?.name ? "" : ""; // se usa order.waUrl
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.75rem" }}>
          ...
          <span style={{ fontSize:8, fontWeight:700, letterSpacing:1.5, color:"#222" }}>
            FKS-{Date.now().toString(36).slice(-6).toUpperCase()}
          </span>
        </div>
        ...
        <div style={{
          ...fade(0.12),
          height:1, background:"linear-gradient(90deg,transparent,#191919,transparent)",
          margin:"1.1rem 0",
        }}/>
        ...
        <div style={{ ...fade(0.15) }}>
          ...
          <p style={{
            textAlign:"center", fontSize:10, color:"#2a2a2a",
        ...
        <div style={{
          ...
          opacity: phase>=3 ? 0.4 : 0, transition:"opacity 0.5s 0.5s ease",
        }}>
          ...
          <span style={{ fontSize:8, color:"#222", letterSpacing:1.5, fontWeight:700 }}>
            COMPRA VERIFICADA · FOKUS ® VENEZUELA
          </span>
        </div>
      </div>
      ...
      <div style={{
        ...
        opacity: phase>=3?1:0, transition:"opacity 0.5s 0.5s ease",
      }}>
        {[
          { href:SOCIAL.instagram, icon:<IcIG s={14}/> },
          { href:SOCIAL.tiktok,    icon:<IcTT s={14}/> },
          { href:SOCIAL.facebook,  icon:<IcFB s={14}/> },
        ].map(({ href, icon }) => (
          <a key={href} href={href} target="_blank" rel="noreferrer"
            style={{ display:"flex",alignItems:"center",justifyContent:"center",width:34,height:34,borderRadius:"50%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",textDecoration:"none" }}>
            {icon}
          </a>
        ))}
      </div>
...
export default function Home() {
  ...
  const [orderSnap,setOrderSnap]     = useState<OrderSnapshot|null>(null);   // ← NUEVO
  ...
  useEffect(() => {
    ...
    upd(); const ro = new ResizeObserver(upd);
    if (navRef.current) ro.observe(navRef.current);
    return () => ro.disconnect();
  }, [mainView,lentesOpen,searchOpen]);

  // Admin state (condensed)
  ...
  const catCounts = useMemo(() => {
    ...
    products.forEach(p => { const mc=p.category.startsWith("LENTES·")?"LENTES":p.category; counts[mc]=(counts[mc]||0)+1; if(p.category.startsWith("LENTES·"))counts[p.category]=(counts[p.category]||0)+1; });
    return counts;
  }, [products]);
  ...
  // ★ ACCIÓN COMPRAR → muestra thank you
  const handleSendOrder = useCallback(() => {
    ...
    setComprobante("");
    setPayMethod(null);
    setDeliveryInfo({ zone:"",nombre:"",cedula:"",telefono:"",agencia:"",direccion:"",estado:"" });
    setMainView("thankyou");
  ...
  const submitProduct = async()=>{setFErr("");setFOk("");if(!fName.trim()||!fPrice||!fCat){setFErr("Nombre, precio y categoría son obligatorios.");return;}if(!editing&&!fFile){setFErr("Selecciona una imagen.");return;}if(!fbReady){setFErr("Firebase no configurado.");return;}setFLoad(true);try{let imgUrl=fPrev;if(fFile)imgUrl=await uploadImg(fFile);const data={name:fName.trim(),description:fDesc.trim(),price:parseFloat(fPrice),category:fCat.toUpperCase(),img:imgUrl};if(editing){await fsUpdate(editing.id,data);setFOk("✓ Producto actualizado");}else{await fsAdd(data);setFOk("✓ Producto agregado");}await loadProducts();setTimeout(resetForm,1800);}catch(err){setFErr("Error: "+(err instanceof Error?err.message:"desconocido"));}finally{setFLoad(false);};};
  ...
  // ── THANK YOU — render directo sin navbar ──
  if (isTY && orderSnap) {
    return (
      <>
        <style>{`
            ...
            <div style={{background:"#111",borderRadius:16,padding:"1.75rem 1.5rem",border:"1px solid #1a1a1a",marginBottom:"1rem"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:"1.25rem",marginBottom:"1.5rem"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  {editingName?(<div style={{display:"flex",gap:"0.5rem",alignItems:"center",marginBottom:"0.5rem",flexWrap:"wrap"}}><input autoFocus value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleSaveName()} placeholder="Nuevo nombre" style={{...S.input,padding:"0.5rem 0.75rem",fontSize:14,flex:1,minWidth:0}}/><button onClick={handleSaveName} disabled={nameLoading||!newName.trim()} style={{background:"#fff",color:"#080808",border:"none",borderRadius:8,padding:"0.5rem 0.85rem",cursor:nameLoading||!newName.trim()?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:4,fontSize:12,fontWeight:800,WebkitTapHighlightColor:"transparent",opacity:nameLoading||!newName.trim()?0.5:1}}><IcCheck s={14} c="#080808"/> Guardar</button><button onClick={()=>setEditingName(false)} style={{background:"none",border:"1px solid #2a2a2a",color:"#555",borderRadius:8,padding:"0.5rem 0.85rem",cursor:"pointer",fontSize:12,WebkitTapHighlightColor:"transparent"}}>Cancelar</button></div>):(<div style={{display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"3px"}}><p style={{margin:0,fontSize:18,fontWeight:800,color:C.text,wordBreak:"break-word"}}>{currentUser.displayName}</p><button onClick={()=>{setNewName(currentUser.displayName);setEditingName(true);}} style={{background:"none",border:"none",cursor:"pointer",padding:4,WebkitTapHighlightColor:"transparent",flexShrink:0,color:"#555",display:"flex",alignItems:"center"}}><IcEdit s={14} c="#555"/></button></div>)}
            ...
            {cart.length===0?(
              ...
              <>
                ...
                <div style={{marginTop:"2rem",background:"#0e0e0e",padding:"1.5rem",borderRadius:12,border:`1px solid ${C.border}`}}>
                  ...
                  {/* ★ BOTÓN → ya no abre WA directamente, primero muestra ThankYou */}
                  <button
                    onClick={handleSendOrder}
                    style={{
                      display:"flex", alignItems:"center", justifyContent:"center", gap:"0.75rem",
                      width:"100%", marginTop:"1.25rem",
                      background: canSendOrder ? "#25D366" : "#1a1a1a",
