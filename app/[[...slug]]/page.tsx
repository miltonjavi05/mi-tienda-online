...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .pv { animation: viewIn 0.25s ease-out both; }
  @media(hover:hover) and (pointer:fine){
    @keyframes viewInDesktop { 0%{opacity:0; transform:translateY(10px) scale(0.99); filter:blur(4px);} 60%{filter:blur(0px);} 100%{opacity:1; transform:translateY(0) scale(1); filter:blur(0);} }
    .pv { animation: viewInDesktop 0.5s cubic-bezier(0.16,1,0.3,1) both; will-change: transform, opacity; }
  }
      ...
      {mainView==="fokus"&&(
        <main className="pv" style={{paddingTop:navH,background:C.bg}}>
          <div className="home-hero" style={{maxWidth:760,margin:"0 auto",padding:"1.25rem 1.5rem 0",textAlign:"center",animation:"slideUp 0.5s ease"}}>
            ...
            <p style={{fontSize:12,color:"#333",lineHeight:1.7,maxWidth:420,margin:"0 auto 2rem"}}>Fokus Accesorios es una aplicación de comercio electrónico (tienda en línea) operada por Milton Javier Chirel Chirel en Venezuela, donde los usuarios pueden comprar lentes de sol, relojes, pulseras, collares, aretes, anillos y billeteras. Esta aplicación utiliza el inicio de sesión con Google exclusivamente para crear una cuenta de usuario, guardar el nombre y la foto de perfil, y agilizar el proceso de compra. Los pedidos se confirman directamente por WhatsApp con el vendedor.</p>
