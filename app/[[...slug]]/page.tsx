...
const CommunityCard=memo(function CommunityCard({post,onClick,index}:{post:typeof COMMUNITY_POSTS[0];onClick:()=>void;index:number}){
  ...
  const ref=useRef<HTMLDivElement>(null);
  useEffect(()=>{const el=ref.current;if(!el)return;const obs=new IntersectionObserver(([e])=>{setVis(e.isIntersecting);},{rootMargin:"-6% 0px -6% 0px",threshold:0.01});obs.observe(el);return()=>obs.disconnect();},[]);
  ...
  return(
    <div ref={ref} onClick={onClick} style={{cursor:"pointer",opacity:vis?1:0,transform:vis?"translateX(0) scale(1) rotate(0deg)":"translateX(-72px) scale(0.94) rotate(-2deg)",filter:vis?"blur(0px)":"blur(6px)",transition:`opacity 0.6s cubic-bezier(0.19,1,0.22,1) ${delay}ms, transform 0.65s cubic-bezier(0.19,1,0.22,1) ${delay}ms, filter 0.6s cubic-bezier(0.19,1,0.22,1) ${delay}ms`,willChange:"transform,opacity,filter",borderRadius:16,overflow:"hidden",background:"#0d0d0d",border:"1px solid #1a1a1a",position:"relative"}} className="cc">
...
const GrabadoCard=memo(function GrabadoCard({item,index}:{item:{img:string;caption:string};index:number}){
  ...
  const delay=Math.min(index*70,260);
  return(
    <div ref={ref} style={{borderRadius:14,overflow:"hidden",background:"#0d0d0d",border:"1px solid #1a1a1a",position:"relative",aspectRatio:"1",cursor:"pointer",opacity:vis?1:0,transform:vis?"translateX(0) scale(1) rotate(0deg)":"translateX(72px) scale(0.94) rotate(2deg)",filter:vis?"blur(0px)":"blur(6px)",transition:`opacity 0.6s cubic-bezier(0.19,1,0.22,1) ${delay}ms, transform 0.65s cubic-bezier(0.19,1,0.22,1) ${delay}ms, filter 0.6s cubic-bezier(0.19,1,0.22,1) ${delay}ms`,willChange:"transform,opacity,filter"}} className="cc">
