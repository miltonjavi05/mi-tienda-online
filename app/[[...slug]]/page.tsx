...
const HCard=memo(function HCard({product,onClick,onBuyNow,index,fmtPrice,isFav,onToggleFavorite,animate=true}:{product:Product;onClick:()=>void;onBuyNow:()=>void;index?:number;fmtPrice:(n:number)=>string;isFav?:boolean;onToggleFavorite?:(id:string)=>void;animate?:boolean|"premium"}){
  ...
  const premiumScale=0.8+ratio*0.24;
  const premiumY=(1-ratio)*15;
  const premiumFilter=`brightness(${(0.8+ratio*0.22).toFixed(3)}) saturate(${(0.82+ratio*0.28).toFixed(3)}) contrast(${(0.96+ratio*0.08).toFixed(3)})`;
  return(
    <div ref={revealRef} className="hc" style={isPremium?{flexShrink:0,width:152,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",display:"flex",flexDirection:"column",opacity:1,transform:`translateY(${premiumY}px) scale3d(${premiumScale},${premiumScale},1) translatez(0)`,filter:premiumFilter,transition:"transform 0.16s cubic-bezier(0.22,1,0.36,1), filter 0.16s cubic-bezier(0.22,1,0.36,1)",willChange:"transform,filter",backfaceVisibility:"hidden"} as React.CSSProperties:animate?{flexShrink:0,width:152,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",display:"flex",flexDirection:"column",opacity:vis?1:0,transform:vis?"translateY(0) scale(1)":"translateY(18px) scale(0.88)",filter:vis?"blur(0px)":"blur(5px)",transition:`opacity 0.5s cubic-bezier(0.19,1,0.22,1) ${revealDelay}ms, transform 0.55s cubic-bezier(0.19,1,0.22,1) ${revealDelay}ms, filter 0.5s cubic-bezier(0.19,1,0.22,1) ${revealDelay}ms`,willChange:"transform,opacity,filter"}:{flexShrink:0,width:152,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",display:"flex",flexDirection:"column",opacity:1,transform:"none",filter:"none"}}>
      <div onClick={onClick} style={{background:"#111",width:152,height:152,overflow:"hidden",marginBottom:"0.55rem",borderRadius:10,position:"relative",cursor:"pointer"}}>
        <div className="iz" style={{width:"100%",height:"100%"}}><LazyImg src={product.img} alt={product.name}/></div>
        <div className="io" style={{position:"absolute",inset:0,background:"rgba(0,0,0,0)",pointerEvents:"none"}}/>
        {isPremium&&<div style={{position:"absolute",inset:0,pointerEvents:"none",borderRadius:10,opacity:Math.max(0,(ratio-0.55)/0.45),transition:"opacity 0.16s cubic-bezier(0.22,1,0.36,1)",background:"linear-gradient(125deg,rgba(255,255,255,0.32) 0%,rgba(255,255,255,0.06) 30%,transparent 55%,rgba(255,255,255,0.1) 78%,rgba(255,255,255,0.28) 100%)",mixBlendMode:"screen" as any}}/>}
        {isPremium&&<div style={{position:"absolute",inset:0,pointerEvents:"none",borderRadius:10,boxShadow:`inset 0 0 0 1px rgba(255,255,255,${(0.08+ratio*0.34).toFixed(3)}), 0 ${Math.round(4+ratio*18)}px ${Math.round(10+ratio*30)}px rgba(255,255,255,${(ratio*0.16).toFixed(3)})`,transition:"box-shadow 0.16s cubic-bezier(0.22,1,0.36,1)"}}/>}
