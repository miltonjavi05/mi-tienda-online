...
async function trackLeadWhatsApp(
  source: string,
...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .ts, .hr { scrollbar-width: none; -ms-overflow-style: none; overscroll-behavior-x: contain; }
  .ts::-webkit-scrollbar, .hr::-webkit-scrollbar { display: none; width: 0; height: 0; }
  ...
  @keyframes laserLineMove { 0%{left:-4%;opacity:0;} 6%{opacity:1;} 90%{opacity:1;} 100%{left:104%;opacity:0;} }
  ...
  @keyframes focusCornerIn { 0%{opacity:0;transform:scale(1.6);} 55%{opacity:1;} 100%{opacity:1;transform:scale(1);} }
...
function DraggableWA(){
  ...
  const finishDrag=useCallback(()=>{if(!dragging.current)return;dragging.current=false;if(raf.current){cancelAnimationFrame(raf.current);raf.current=0;flush();}setPressed(false);if(moved.current){const s=snapToEdge(live.current.x,live.current.y);live.current=s;setSnapping(true);setPos({...s});setTimeout(()=>setSnapping(false),420);}else{setPos({...live.current});}},[snapToEdge,flush]);
   const onClick=useCallback((e:React.MouseEvent)=>{if(moved.current){e.preventDefault();return;}trackLeadWhatsApp("boton_flotante");const msg=encodeURIComponent("Hola! Vi su tienda Fokus y quiero más información 🖤");window.location.href=`https://wa.me/${WHATSAPP_NUMBER}?text=${msg}`;},[]);
  ...
  return(
    <div ref={revealRef} className={isPremium?"hc hc-scroll-focus":"hc"} style={isPremium?{flexShrink:0,width:152,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",display:"flex",flexDirection:"column",borderRadius:14}:animate?{flexShrink:0,width:152,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",display:"flex",flexDirection:"column",opacity:vis?1:0,transform:vis?"translateY(0) scale(1)":"translateY(18px) scale(0.88)",filter:vis?"blur(0px)":"blur(5px)",transition:`opacity 0.5s cubic-bezier(0.19,1,0.22,1) ${revealDelay}ms, transform 0.55s cubic-bezier(0.19,1,0.22,1) ${revealDelay}ms, filter 0.5s cubic-bezier(0.19,1,0.22,1) ${revealDelay}ms`,willChange:"transform,opacity,filter"}:{flexShrink:0,width:152,WebkitTapHighlightColor:"transparent",touchAction:"manipulation",display:"flex",flexDirection:"column",opacity:1,transform:"none",filter:"none"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"0.3rem"}}>
        ...
        <button
          onClick={e=>{e.stopPropagation();trackLeadWhatsApp("finalizar_pedido",{value:getFinalPrice(product),content_ids:[product.id],content_name:product.name});window.location.href=buildConsultUrl(product);}}
    ...
    <div ref={ref} style={{flexShrink:0,overflow:"visible",opacity:vis?1:0,transform:vis?"translateY(0) scale(1)":"translateY(18px) scale(0.88)",filter:vis?"blur(0px)":"blur(5px)",transition:`opacity 0.5s cubic-bezier(0.19,1,0.22,1) ${delay}ms, transform 0.55s cubic-bezier(0.19,1,0.22,1) ${delay}ms, filter 0.5s cubic-bezier(0.19,1,0.22,1) ${delay}ms`,willChange:"transform,opacity,filter"}}>
      <button onClick={onClick} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:8,fontFamily:"inherit",WebkitTapHighlightColor:"transparent",outline:"none",width:66,touchAction:"manipulation"}}>
        <div className="pc" style={{width:64,height:64,borderRadius:"50%",overflow:"hidden",position:"relative",background:"#111",boxShadow:"0 0 0 1px rgba(255,255,255,0.16), 0 0 0 4px rgba(255,255,255,0.035), 0 10px 26px rgba(0,0,0,0.55)"}}>
          ...
          <div className="io" style={{position:"absolute",inset:0}}/>
          {vis&&<div key={cycle} style={{position:"absolute",inset:0,pointerEvents:"none",overflow:"hidden",borderRadius:"50%"}}><div style={{position:"absolute",top:0,left:0,width:"60%",height:"100%",background:"linear-gradient(120deg,transparent 0%,rgba(255,255,255,0.22) 45%,rgba(255,255,255,0.06) 55%,transparent 100%)",filter:"blur(1px)",animation:`cardSweep 1s cubic-bezier(0.19,1,0.22,1) ${delay+70}ms both`}}/></div>}
        </div>
...
// ─── REVEAL UP (SCROLL, ABAJO HACIA ARRIBA, ULTRA PREMIUM) ────────────────────
const RevealUp=memo(function RevealUp({children,delay=0}:{children:React.ReactNode;delay?:number}){
  const[vis,setVis]=useState(false);
  const ref=useRef<HTMLDivElement>(null);
  useEffect(()=>{
    const el=ref.current;
    if(!el)return;
    const obs=new IntersectionObserver(([entry])=>{if(entry.isIntersecting)setVis(true);},{rootMargin:"-10% 0px -10% 0px",threshold:0.01});
    obs.observe(el);
    return()=>obs.disconnect();
  },[]);
   return(
...
const CommunityCard=memo(function CommunityCard({post,onClick,index}:{post:typeof COMMUNITY_POSTS[0];onClick:()=>void;index:number}){
  ...
  return(
    <div ref={ref} onClick={onClick} style={{cursor:"pointer",transformOrigin:isLeft?"left center":"right center",opacity:vis?1:0,transform:vis?"rotateY(0deg) translateZ(0) translateY(0)":`rotateY(${isLeft?-82:82}deg) translateZ(-60px) translateY(14px)`,transition:`opacity 0.6s cubic-bezier(0.22,1,0.36,1) ${delay}ms, transform 0.75s cubic-bezier(0.22,1,0.36,1) ${delay}ms`,borderRadius:16,overflow:"hidden",background:"#0d0d0d",border:"1px solid #1a1a1a",position:"relative",willChange:"transform,opacity",boxShadow:vis?"0 14px 34px rgba(0,0,0,0.5)":"none"}} className="cc">
      <div style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:5,background:isLeft?"linear-gradient(90deg, rgba(0,0,0,0.6) 0%, transparent 32%)":"linear-gradient(270deg, rgba(0,0,0,0.6) 0%, transparent 32%)",opacity:vis?0:1,transition:`opacity 0.5s ease ${delay}ms`}}/>
      {vis&&<div key={cycle} style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:4,overflow:"hidden",borderRadius:16}}><div style={{position:"absolute",top:0,left:0,width:"60%",height:"100%",background:"linear-gradient(120deg,transparent 0%,rgba(255,255,255,0.16) 45%,rgba(255,255,255,0.05) 55%,transparent 100%)",filter:"blur(1.5px)",animation:`cardSweep 1s cubic-bezier(0.19,1,0.22,1) ${delay+140}ms both`}}/></div>}
...
// ─── LASER CARD (GRABADOS) ────────────────────────────────────────────────────
const LaserCard=memo(function LaserCard({item,index}:{item:{img:string;caption:string};index:number}){
  const[vis,setVis]=useState(false);
  const[cycle,setCycle]=useState(0);
  const wasVis=useRef(false);
  const ref=useRef<HTMLDivElement>(null);
  ...
  const delay=Math.min(index*110,330);
  return(
    <div ref={ref} className="cc" style={{borderRadius:14,overflow:"hidden",background:"#0d0d0d",border:`1px solid ${vis?"#3a2a1a":"#1a1a1a"}`,position:"relative",aspectRatio:"1",cursor:"pointer",opacity:vis?1:0,transform:vis?"translateY(0) scale(1)":"translateY(16px) scale(0.96)",transition:`opacity 0.4s cubic-bezier(0.19,1,0.22,1) ${delay}ms, transform 0.45s cubic-bezier(0.19,1,0.22,1) ${delay}ms, border-color 0.6s ease ${delay+400}ms`,willChange:"transform,opacity"}}>
      <div style={{position:"absolute",inset:0,clipPath:vis?"inset(0 0 0 0)":"inset(0 100% 0 0)",transition:`clip-path 0.65s cubic-bezier(0.65,0,0.35,1) ${delay+50}ms`}}>
        <img src={item.img} alt={item.caption} loading="lazy" style={{width:"100%",height:"100%",objectFit:"cover",display:"block",pointerEvents:"none",filter:vis?"brightness(1) contrast(1) saturate(1)":"brightness(2.6) contrast(1.4) saturate(0.6)",transition:`filter 0.65s cubic-bezier(0.65,0,0.35,1) ${delay+50}ms`} as React.CSSProperties} draggable={false}/>
      </div>
      {vis&&<div key={"beam"+cycle} style={{position:"absolute",top:-6,bottom:-6,width:3,background:"linear-gradient(180deg,transparent 0%,#ff7a1f 10%,#fff 50%,#ff7a1f 90%,transparent 100%)",boxShadow:"0 0 10px 2px rgba(255,120,40,0.9), 0 0 26px 8px rgba(255,70,0,0.5)",animation:`laserLineMove 0.65s cubic-bezier(0.65,0,0.35,1) ${delay+50}ms both`,pointerEvents:"none",zIndex:3}}/>}
      {vis&&<div key={"glow"+cycle} style={{position:"absolute",top:0,bottom:0,width:34,marginLeft:-16,background:"radial-gradient(ellipse at center, rgba(255,110,30,0.5) 0%, rgba(255,60,0,0) 72%)",filter:"blur(3px)",mixBlendMode:"screen" as any,animation:`laserLineMove 0.65s cubic-bezier(0.65,0,0.35,1) ${delay+50}ms both`,pointerEvents:"none",zIndex:2}}/>}
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,0.7) 0%,transparent 60%)",pointerEvents:"none",zIndex:1}}/>
      <p style={{position:"absolute",bottom:10,left:12,right:12,margin:0,fontSize:11,fontWeight:700,color:"#fff",lineHeight:1.3,zIndex:2,opacity:vis?1:0,transform:vis?"translateY(0)":"translateY(6px)",transition:`opacity 0.4s ease ${delay+420}ms, transform 0.4s ease ${delay+420}ms`}}>{item.caption}</p>
    </div>
  );
});
  ...
  return(
    <div draggable onDragStart={()=>onDragStart(p.id)} onDragOver={e=>{e.preventDefault();onDragOver(p.id);}} onDragEnd={onDragEnd} data-rowid={p.id} className="ar" style={{display:"flex",alignItems:"center",gap:"0.75rem",padding:"0.6rem 0.65rem",borderRadius:8,background:isOver?"#1e1e1e":editing?.id===p.id?"#1a1a1a":"transparent",opacity:isDragging?0.4:1,border:isOver?"1px dashed #3a3a3a":"1px solid transparent",transition:"opacity 0.15s, background 0.15s, border 0.15s",cursor:"default",userSelect:"none",WebkitUserSelect:"none"}}>
      ...
      <div style={{display:"flex",gap:"0.3rem",flexShrink:0,flexWrap:"wrap",justifyContent:"flex-end",maxWidth:170}}>
        <button onClick={()=>onEditStock(p)} title="Editar stock" style={{background:(p.stock??0)===0?"#1e0d0d":"#111",color:(p.stock??0)===0?"#ff8888":"#888",border:`1px solid ${(p.stock??0)===0?"#3a1515":"#222"}`,padding:"0.3rem 0.5rem",borderRadius:6,cursor:"pointer",fontSize:10,fontFamily:"inherit",fontWeight:700,WebkitTapHighlightColor:"transparent"}}>📦 {p.stock??0}</button>
...
const[deliveryInfo,setDeliveryInfo]=useState<DeliveryInfo>({zone:"",nombre:"",cedula:"",telefono:"",agencia:"",direccion:"",estado:""});
 ...
 useEffect(()=>{initMetaPixel();getFbcFromUrl();},[]);
  ...
  const[adminSec,setAdminSec]=useState<"menu"|"products"|"sales"|"stats"|"coupons"|"lookup"|"comments"|"bulkedit"|"bulkadd"|"inventory"|"agenda"|"interest">("menu");
  const adminSecRef=useRef(adminSec);
  useEffect(()=>{adminSecRef.current=adminSec;},[adminSec]);
  const enterAdminSec=useCallback((sec:"products"|"sales"|"stats"|"coupons"|"lookup"|"comments"|"bulkedit"|"bulkadd"|"inventory"|"agenda"|"interest")=>{
...
const couponsLoaded=useRef(false);
  ...
  const catThumbs=useMemo(()=>{const thumbs:Record<string,string>={};products.filter(p=>p.active!==false).forEach(p=>{if(!thumbs[p.category])thumbs[p.category]=p.img;const mc=p.category.startsWith("LENTES·")?"LENTES":p.category;if(!thumbs[mc])thumbs[mc]=p.img;});return thumbs;},[products]);
...
const removeCoupon=useCallback(()=>{setAppliedCoupon(null);setCouponInput("");setCouponCheckErr("");},[]);
  ...
  const loadInterest=useCallback(async(force=false)=>{
    if(interestLoaded.current&&!force)return;
    setInterestLoading(true);
    try{
      const raw=await fsGetCollection("product_interest",1000);
      setInterestList(raw);
      interestLoaded.current=true;
    }catch{ /* silencioso */ }
...
const filteredComments=useMemo(()=>{
  ...
  const interestAgg=useMemo(()=>{
    ...
    const byCarts=[...list].sort((a,b)=>b.carts-a.carts);
        ...
        <main className="pv" style={{paddingTop:navH,background:C.bg}}>
          <div className="home-hero" style={{maxWidth:760,margin:"0 auto",padding:"1.25rem 1.5rem 0",textAlign:"center",animation:"slideUp 0.5s ease"}}>
            ...
            <div style={{position:"relative",display:"inline-block",marginBottom:"2rem",borderRadius:8}}>
              <span aria-hidden="true" style={{position:"absolute",top:-10,left:-10,width:16,height:16,borderTop:"2px solid rgba(255,255,255,0.85)",borderLeft:"2px solid rgba(255,255,255,0.85)",borderRadius:"3px 0 0 0",animation:"focusCornerIn 0.7s cubic-bezier(0.16,1,0.3,1) both, focusCornerBreathe 2.6s ease-in-out 0.7s infinite",pointerEvents:"none"}}/>
              <span aria-hidden="true" style={{position:"absolute",top:-10,right:-10,width:16,height:16,borderTop:"2px solid rgba(255,255,255,0.85)",borderRight:"2px solid rgba(255,255,255,0.85)",borderRadius:"0 3px 0 0",animation:"focusCornerIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.05s both, focusCornerBreathe 2.6s ease-in-out 0.75s infinite",pointerEvents:"none"}}/>
              <span aria-hidden="true" style={{position:"absolute",bottom:-10,left:-10,width:16,height:16,borderBottom:"2px solid rgba(255,255,255,0.85)",borderLeft:"2px solid rgba(255,255,255,0.85)",borderRadius:"0 0 0 3px",animation:"focusCornerIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.1s both, focusCornerBreathe 2.6s ease-in-out 0.8s infinite",pointerEvents:"none"}}/>
              <span aria-hidden="true" style={{position:"absolute",bottom:-10,right:-10,width:16,height:16,borderBottom:"2px solid rgba(255,255,255,0.85)",borderRight:"2px solid rgba(255,255,255,0.85)",borderRadius:"0 0 3px 0",animation:"focusCornerIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.15s both, focusCornerBreathe 2.6s ease-in-out 0.85s infinite",pointerEvents:"none"}}/>
              <span aria-hidden="true" style={{position:"absolute",inset:-10,borderRadius:14,border:"1px solid rgba(255,255,255,0.6)",animation:"focusRingPulse 2.6s ease-out infinite",pointerEvents:"none"}}/>
              <button onClick={()=>{setMainView("shop");setShopFilter("TODO");}} style={{position:"relative",overflow:"hidden",background:"#ffffff",color:"#080808",border:"none",fontSize:11,fontWeight:900,letterSpacing:3,padding:"1.15rem 2.9rem",borderRadius:8,cursor:"pointer",fontFamily:"inherit",WebkitTapHighlightColor:"transparent",boxShadow:"0 16px 40px rgba(255,255,255,0.16), inset 0 1px 0 rgba(255,255,255,1)"}}>
          ...
          <div className="shop-container" style={{maxWidth:1200,margin:"0 auto",padding:"0.75rem 1rem 5rem"}}>
            {loading?(
              ...
              getVisCats().map(cat=>{
                ...
                return(
                  <ScrollFocusSection key={cat} style={{marginBottom:"0.55rem"}}>
                    ...
                    <div style={{marginBottom:"-0.65rem"}}><HRow products={prods} onSelect={openProd} onBuyNow={openProd} fmtPrice={fmtPrice} animate="premium"/></div>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,margin:"1.4rem auto 0",width:"fit-content",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"0.32rem 0.9rem",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",position:"relative",zIndex:2}}>
        ...
        <main className="pv" style={{paddingTop:navH,background:C.bg}}>
          ...
          <div style={{maxWidth:1100,margin:"0 auto",padding:"0 1rem 5rem"}}>
            <div style={{marginBottom:"2rem",background:"linear-gradient(135deg,#0f0f0f 0%,#111 100%)",borderRadius:16,border:"1px solid #1e1e1e",padding:"1.5rem",display:"flex",alignItems:"center",gap:"1rem",flexWrap:"wrap",justifyContent:"space-between"}}>
             ...
             <div className="cg" style={{display:"grid",gap:"0.85rem",perspective:1400}}>
...
{fErr&&<div style={{color:"#ff5555",fontSize:12,background:"#1e0808",padding:"0.65rem 1rem",borderRadius:8}}>{fErr}</div>}{fOk&&<div style={{color:"#55cc77",fontSize:12,background:"#081e0e",padding:"0.65rem 1rem",borderRadius:8}}>{fOk}</div>}<div style={{display:"flex",gap:"0.65rem",flexWrap:"wrap"}}><button onClick={submitProduct} disabled={fLoad} style={{...S.adminBtn,flex:1,opacity:fLoad?0.4:1,cursor:fLoad?"not-allowed":"pointer"}}>{fLoad?"Subiendo...":(editing?"Guardar cambios":"Agregar producto")}</button>{editing&&<button onClick={resetForm} style={{...S.adminBtn,flex:"0 0 auto",width:"auto",padding:"0.8rem 1.1rem",background:"transparent",color:"#444",border:"1px solid #1e1e1e"}}>Cancelar</button>}</div></div></div>
            ...
            {adminLogged&&adminSec==="interest"&&(<>
              ...
              <div style={{display:"flex",gap:"0.5rem",marginBottom:"1.25rem",flexWrap:"wrap"}}>
                ...
                <input placeholder="🔎 Buscar por nombre o código…" value={interestSearch} onChange={e=>setInterestSearch(e.target.value)} style={{...S.input,flex:1,minWidth:160}}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:"0.75rem",marginBottom:"1.5rem"}}>
              ...
              <div style={{background:"#111",borderRadius:14,border:"1px solid #1a1a1a",padding:"1.25rem",marginBottom:"1.25rem"}}>
                ...
                {interestLoading&&!interestAgg.byViews.length?(
                  <p style={{color:"#333",fontSize:12,textAlign:"center",padding:"1rem"}}>Cargando…</p>
              ...
              <div style={{background:"#111",borderRadius:14,border:"1px solid #1a1a1a",padding:"1.25rem"}}>
                <p style={{fontSize:9,fontWeight:800,letterSpacing:2.5,color:"#333",margin:"0 0 1rem"}}>TOP ARTÍCULOS MÁS AGREGADOS AL CARRITO</p>
                {interestLoading&&!interestAgg.byCarts.length?(
                  <p style={{color:"#333",fontSize:12,textAlign:"center",padding:"1rem"}}>Cargando…</p>
            ...
            {adminLogged&&adminSec==="inventory"&&(<>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.5rem"}}><h1 style={{color:"#fff",fontSize:16,fontWeight:900,margin:0,letterSpacing:2}}>INVENTARIO / STOCK</h1><button onClick={()=>exitAdminSec()} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:12,fontFamily:"inherit",WebkitTapHighlightColor:"transparent"}}>← MENÚ</button></div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:"0.75rem",marginBottom:"1.25rem"}}>
                <div style={{background:"#111",border:"1px solid #1a1a1a",borderRadius:12,padding:"0.9rem"}}><p style={{margin:"0 0 3px",fontSize:18,fontWeight:900,color:"#fff"}}>{inventoryStats.totalUnits}</p><p style={{margin:0,fontSize:8,fontWeight:800,letterSpacing:1.5,color:"#444"}}>UNIDADES TOTALES</p></div>
                <div style={{background:"#111",border:"1px solid #3a1a1a",borderRadius:12,padding:"0.9rem"}}><p style={{margin:"0 0 3px",fontSize:18,fontWeight:900,color:"#ff6666"}}>{inventoryStats.outOfStock}</p><p style={{margin:0,fontSize:8,fontWeight:800,letterSpacing:1.5,color:"#444"}}>AGOTADOS</p></div>
                <div style={{background:"#111",border:"1px solid #3a2f10",borderRadius:12,padding:"0.9rem"}}><p style={{margin:"0 0 3px",fontSize:18,fontWeight:900,color:"#ffd43b"}}>{inventoryStats.lowStock}</p><p style={{margin:0,fontSize:8,fontWeight:800,letterSpacing:1.5,color:"#444"}}>STOCK BAJO (≤5)</p></div>
              </div>
              <div style={{background:"#111",borderRadius:12,padding:"1.5rem",border:"1px solid #1a1a1a"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:2}}>
                  {invProds.map(p=>{const st=p.stock??0;const color=st===0?"#ff6666":st<=5?"#ffd43b":"#4caf50";const bg=st===0?"#1e0d0d":st<=5?"#1a1608":"#0d1e0d";return(
                    <div key={p.id} style={{display:"flex",alignItems:"center",gap:"0.75rem",padding:"0.6rem 0.65rem",borderRadius:8}}>
                      ...
                      <input type="number" min="0" defaultValue={st} onBlur={async e=>{const val=Math.max(0,parseInt(e.target.value)||0);if(val===st)return;setProducts(prev=>{const upd=prev.map(x=>x.id===p.id?{...x,stock:val}:x);setCachedProducts(upd);return upd;});try{await fsUpdate(p.id,{stock:val});invalidateProductsCache();}catch{}}} style={{width:64,background:bg,color,border:`1px solid ${color}55`,borderRadius:8,padding:"0.4rem 0.5rem",fontSize:12,fontWeight:800,textAlign:"center",fontFamily:"inherit",flexShrink:0}}/>
                    </div>
                  );})}
                  {!invProds.length&&<p style={{color:"#333",textAlign:"center",padding:"1.5rem",fontSize:12}}>Sin resultados</p>}
                </div>
              </div>
            </>)}
      ...
      {selectedProduct&&(
        <div onClick={closeProdModal} className="pm-overlay" style={{position:"fixed",inset:0,zIndex:400,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"fadeIn 0.18s ease"}}>
          <div onClick={e=>e.stopPropagation()} className="pm-content" style={{background:"#111",width:"100%",maxWidth:520,maxHeight:"92vh",borderRadius:"18px 18px 0 0",display:"flex",flexDirection:"column",animation:"slideUp 0.28s cubic-bezier(0.25,0.46,0.45,0.94)",border:"1px solid #1e1e1e",borderBottom:"none",overflow:"hidden"}}>
            ...
            <div className="pm-footer-bar" style={{position:"sticky",bottom:0,marginLeft:"-1.5rem",marginRight:"-1.5rem",marginTop:"1rem",padding:"1rem 1.5rem 1.5rem",background:"#111",borderTop:"1px solid #1e1e1e",display:"flex",flexDirection:"column",gap:"0.6rem",boxShadow:"0 -8px 24px rgba(0,0,0,0.4)",zIndex:5}}>
              {addedFlash&&(
                <div style={{position:"absolute",bottom:"100%",left:"1.5rem",right:"1.5rem",marginBottom:"0.6rem",display:"flex",alignItems:"center",gap:10,background:"linear-gradient(155deg,rgba(255,255,255,0.14) 0%,rgba(20,20,20,0.96) 45%,rgba(6,6,6,0.99) 100%)",border:"1px solid rgba(255,255,255,0.22)",borderRadius:12,padding:"0.75rem 1rem",animation:"addedBadgeIn 0.22s cubic-bezier(0.22,1,0.36,1)",boxShadow:"0 10px 30px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.15)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",zIndex:5,overflow:"hidden",willChange:"transform,opacity"}}>
                  ...
                  <p style={{position:"relative",margin:0,fontSize:12,fontWeight:900,color:"#fff",letterSpacing:0.3,flex:1,minWidth:0}}>Se han añadido {modalQty} unidad{modalQty>1?"es":""} al carrito</p>
                </div>
              )}
